...
import com.singed.annotation.beans.Yellow;
 ...
 * 1).加了环境标识的bean,只有这个环境被激活的时候，bean才会被注册到容器中。默认是default
 * 2).写在配置上，只有是指定的环境的时候，整个配置类里面的所有配置才会生效
 * 3).没有标注环境标识的bean在任何环境下都会加载
 * @Author : Singed
 * @Date : 2021/9/10 23:32
 */
...
public class MainConfigOfProfile implements EmbeddedValueResolverAware {
    ...
    public void setEmbeddedValueResolver(StringValueResolver resolver) {
        dirverClass = resolver.resolveStringValue("${db.driverClass}");
